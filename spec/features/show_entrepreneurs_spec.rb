...
feature 'Show entrepreneurs' do
  ...
  scenario 'with projects on the homepage' do
    visit('/')
    expect(page).to have_selector('div.project', text: 'Microinsurance for farmers')
  end

  scenario 'with links to a more detailed profile page' do
    visit('/')
  ...
  scenario 'with a button allowing donor to make a Bitcoin donation' do
    visit('/')
    click_link('Log In')
    sign_in
    visit('/')
